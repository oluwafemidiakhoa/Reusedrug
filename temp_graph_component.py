...
path = Path('web-app/components/GraphExplorer.tsx')
...
export default function GraphExplorer({ insight }: Props) {
  ...
  const [showCentralOnly, setShowCentralOnly] = useState<boolean>(false);
  ...
  const elements = useMemo(() => {
    ...
    const highlighted = new Set<string>(showCentralOnly ? insight.top_central_nodes : []);
    ...
    return [...nodes, ...edges];
  }, [insight, predicates, selectedPredicate, showCentralOnly]);
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <label className="flex items-center gap-2 text-xs text-slate-300">
            <input
              ...
              onChange={(event) => setShowCentralOnly(event.target.checked)}
